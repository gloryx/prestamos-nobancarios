...
export function RentabilidadCanceladosPage() {
  ...
  return <main className="rentabilidad-report-page">
      <div className="page-heading">
       <div><p className="eyebrow">CIERRE MENSUAL</p><h1><TrendingUp size={25} aria-hidden="true" /> Reporte de préstamos cancelados</h1><p className="muted">Rentabilidad real agregada, sin detalle individual.</p></div>
    ...
    {!loading && !error && report && <>
      ...
      {!hasData && <div className="panel state-box rentabilidad-empty">No hay préstamos cancelados con transición real dentro del mes seleccionado.</div>}
      {hasData && <>
        <section className="rentabilidad-charts">
          ...
          <div className="panel rentabilidad-chart-panel"><h2>Distribución del capital</h2><div className="rentabilidad-chart"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={rows.filter((row) => row.capital > 0)} dataKey="capital" nameKey="plazo" innerRadius={58} outerRadius={92} paddingAngle={2}>{rows.filter((row) => row.capital > 0).map((row, index) => <Cell key={row.plazo} fill={colors[index % colors.length]} />)}</Pie><Tooltip formatter={(value) => [formatCRC(safeNumber(Number(value))), 'Capital']} /></PieChart></ResponsiveContainer></div></div>
        </section>
        <section className="panel rentabilidad-table-panel"><h2>Rentabilidad agregada por plazo</h2><div className="table-wrap"><table><thead><tr><th>Plazo real</th><th>Préstamos</th><th>Capital</th><th>Ganancia</th><th>Rentabilidad</th><th>Tasa 30 días</th></tr></thead><tbody>{rows.map((row) => <tr key={row.plazo}><td>{row.plazo}</td><td>{row.cantidad}</td><td>{formatCRC(row.capital)}</td><td>{formatCRC(row.ganancia)}</td><td>{safeNumber(row.rentabilidadTotal).toFixed(2)}%</td><td>{safeNumber(row.tasa30Dias).toFixed(2)}%</td></tr>)}<tr className="rentabilidad-total-row"><th>TOTAL</th><th>{summary?.prestamosCancelados ?? 0}</th><th>{formatCRC(safeNumber(summary?.capitalTotal))}</th><th>{formatCRC(safeNumber(summary?.gananciaTotal))}</th><th>{safeNumber(summary?.rentabilidadTotal).toFixed(2)}%</th><th>{safeNumber(summary?.tasa30Dias).toFixed(2)}%</th></tr></tbody></table></div></section>
