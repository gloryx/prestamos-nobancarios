...
import { cambiarEstadoPrestamo, listarCandidatosIncobrables, listarIncobrables } from '../application/prestamos.use-cases'
import type { IncobrableLoan, IncobrablesFilters, IncobrablesPage } from '../domain/prestamo.types'
...
export function GestionIncobrablesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { setPage(tab === 'vencidos' ? await listarCandidatosIncobrables(repository, filters) : await listarIncobrables(repository, filters)) } catch (cause) { setError(prestamoErrorMessage(cause)) } finally { setLoading(false) }
  }, [filters, tab])
  ...
  return <section className="gestion-incobrables-page">
    ...
    <div className="panel incobrables-toolbar"><label>Buscar cliente<input value={search} onChange={(event) => updateSearch(event.target.value)} placeholder="Identificación o nombre" /></label><label>Fecha de referencia<input type="date" value={filters.fechaReferencia ?? ''} onChange={(event) => setFilters((current) => ({ ...current, fechaReferencia: event.target.value, pagina: 1 }))} /></label><Search size={18} aria-hidden="true" /></div>
