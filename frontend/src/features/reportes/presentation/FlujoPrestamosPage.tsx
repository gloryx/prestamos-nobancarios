...
export function FlujoPrestamosPage() {
  ...
  return <main className="flujo-report-page">
    ...
    {loading && <div className="panel state-box">Cargando reporte...</div>}
    {!loading && error && <div className="panel state-box flujo-error" role="alert">{error}{!periodError && <button className="secondary-button" onClick={retry}><RefreshCw size={15} /> Reintentar</button>}</div>}
    {!loading && !error && report && <>
      <section className="flujo-status panel" aria-live="polite">{warningMonths.length === 0 ? <><CircleCheck size={17} aria-hidden="true" /> Conciliación correcta</> : <><CircleAlert size={17} aria-hidden="true" /> <span>Conciliación con diferencias en: <strong>{warningMonths.map((row) => row.periodo).join(', ')}</strong></span></>}</section>
...
function Composition({ total }: { total: FlujoTotal }) {
  if (total.pagosRecibidos === 0) return <section className="panel flujo-composition"><h2>Composición de pagos</h2><p>Sin pagos registrados</p></section>
  const items = [['Capital recuperado', total.capitalRecuperado, total.porcentajeCapitalPagos], ['Interés cobrado', total.gananciaRealizada, total.porcentajeInteresPagos]] as Array<[string, number, number | null]>
  return <section className="panel flujo-composition"><h2>Composición de pagos</h2>{items.map(([label, value, percentage]) => <div className="composition-item" key={label}><div><span>{label}</span><strong>{money(value)}</strong></div>{percentage !== null && <div className="composition-bar" aria-label={`${label}: ${percentage}%`}><span style={{ width: `${Math.max(0, Math.min(100, percentage))}%` }} /><small>{percentage.toLocaleString('es-CR', { maximumFractionDigits: 1 })}%</small></div>}</div>)}</section>
}

type ChartRow = Partial<FlujoMes> & { etiqueta: string }
function Chart({ title, data, lines }: { title: string; data: ChartRow[]; lines: Array<[string, string, string]> }) {
  const chartData = data.map((row) => ({ ...row, etiqueta: row.etiqueta || monthNames[(row.mes ?? 1) - 1] }))
  return <div className="panel flujo-chart"><h2>{title}</h2><ResponsiveContainer width="100%" height={270}><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="etiqueta" /><YAxis tickFormatter={(value) => compactMoney(Number(value))} /><Tooltip formatter={(value) => money(Number(value))} /><Legend />{lines.map(([key, label, color]) => <Line key={key} type="monotone" dataKey={key} name={label} stroke={color} strokeWidth={2} dot={false} />)}</LineChart></ResponsiveContainer></div>
}

const columns: Array<[string, keyof FlujoMes]> = [['Capital colocado', 'capitalColocado'], ['Pagos recibidos', 'pagosRecibidos'], ['Capital recuperado', 'capitalRecuperado'], ['Ganancia realizada', 'gananciaRealizada'], ['Flujo neto', 'flujoNeto']]
function MonthlyTable({ rows, total }: { rows: FlujoMes[]; total: FlujoTotal }) { return <div className="table-wrap"><table><thead><tr><th>Mes</th>{columns.map(([label]) => <th key={label}>{label}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.periodo}><th>{monthNames[row.mes - 1]}</th>{columns.map(([, key]) => <td key={key}>{key === 'flujoNeto' ? signedMoney(row[key]) : money(row[key])}</td>)}</tr>)}<tr className="flujo-total-row"><th>TOTAL</th>{columns.map(([, key]) => <th key={key}>{key === 'flujoNeto' ? signedMoney(total[key]) : money(total[key])}</th>)}</tr></tbody></table></div> }
function AnnualTable({ report }: { report: FlujoPrestamosReport }) { return <div className="table-wrap"><table><thead><tr><th>Año</th>{columns.map(([label]) => <th key={label}>{label}</th>)}</tr></thead><tbody>{Object.entries(report.anuales).map(([year, row]) => <tr key={year}><th>{year}</th>{columns.map(([, key]) => <td key={key}>{key === 'flujoNeto' ? signedMoney(row[key]) : money(row[key])}</td>)}</tr>)}<tr className="flujo-total-row"><th>TOTAL DEL PERÍODO</th>{columns.map(([, key]) => <th key={key}>{key === 'flujoNeto' ? signedMoney(report.total[key]) : money(report.total[key])}</th>)}</tr></tbody></table></div> }
