...
type EditablePlanRow = { id?: number; fechaVencimiento: string; montoProgramado: number | null; editable: boolean; protected: boolean; original?: PlanPago }
...
function PersonalizePlanModal({ loan, plan, summary, onClose, onSaved }: { loan: Prestamo; plan: PlanPago[]; summary: PagoResumen; onClose: () => void; onSaved: () => Promise<void> }) {
  const [rows, setRows] = useState<EditablePlanRow[]>(() => plan.map(item => ({ id: item.id, fechaVencimiento: item.fechaVencimiento.slice(0, 10), montoProgramado: item.montoProgramado, editable: item.editable === true, protected: item.editable !== true, original: item })))
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  useLocalModalAccessibility(true, onClose, saving)

  const editableRows = rows.filter(row => row.editable)
  const totalCents = editableRows.reduce((total, row) => total + Math.round((row.montoProgramado ?? 0) * 100), 0)
  const balanceCents = Math.round(summary.saldoPendiente * 100)
  const differenceCents = balanceCents - totalCents
  const basicValid = rows.every(row => !row.editable || (row.montoProgramado != null && Number.isFinite(row.montoProgramado) && row.montoProgramado > 0 && hasAtMostTwoDecimals(row.montoProgramado) && isValidDate(row.fechaVencimiento)))
  const chronological = rows.every((row, index) => index === 0 || row.fechaVencimiento > rows[index - 1].fechaVencimiento)
  const noSundays = rows.every(row => !row.editable || !isSunday(row.fechaVencimiento))
  const valid = differenceCents === 0 && basicValid && chronological && noSundays && !saving
  ...
  const addRow = () => setRows(current => [...current, { fechaVencimiento: current.at(-1)?.fechaVencimiento ?? today(), montoProgramado: null, editable: true, protected: false }])
  const removeRow = (index: number) => setRows(current => current.filter((_, rowIndex) => rowIndex !== index))
  const save = async () => {
    if (!valid) return
    setSaving(true)
    setError('')
    const input: PersonalizarPlanPagoInput = { cuotas: editableRows.map(row => ({ ...(row.id === undefined ? {} : { id: row.id }), fechaVencimiento: row.fechaVencimiento, montoProgramado: row.montoProgramado! })) }
    try {
  ...
  return <div className="payment-modal-backdrop"><div className="payment-modal plan-personalization-modal" role="dialog" aria-modal="true" aria-labelledby="plan-personalization-title">
    ...
    <div className="table-wrap plan-personalization-table-wrap"><table className="plan-personalization-table"><thead><tr><th>N°</th><th>Fecha</th><th>Monto</th><th>Tipo</th><th aria-label="Acciones" /></tr></thead><tbody>{rows.map((row, index) => <tr key={row.id ?? `new-${index}`} className={row.protected ? 'plan-protected-row' : undefined}><td>{index + 1}</td><td>{row.editable ? <input type="date" value={row.fechaVencimiento} onChange={event => updateRow(index, { fechaVencimiento: event.target.value })} aria-label={`Fecha de cuota ${index + 1}`} /> : <span>{dateLabel(row.fechaVencimiento)}</span>}</td><td>{row.editable ? <CurrencyInput value={row.montoProgramado} onChange={value => updateRow(index, { montoProgramado: value })} inputMode="decimal" aria-label={`Monto de cuota ${index + 1}`} /> : <span>{money(row.montoProgramado ?? 0)}</span>}</td><td>{row.protected ? <span className="plan-protected-label" title="Cuota protegida por historial de pagos" aria-label="Cuota protegida por historial de pagos"><Lock size={14} aria-hidden="true" /> Protegida</span> : <span>Operativa</span>}</td><td>{row.editable && <button className="table-action" type="button" onClick={() => removeRow(index)} aria-label={`Eliminar cuota ${index + 1}`} title="Eliminar cuota"><Trash2 size={16} /></button>}</td></tr>)}</tbody></table></div>
    ...
    {!basicValid && <p className="form-error" role="alert">Revisá que cada cuota editable tenga un monto mayor que cero, máximo dos decimales y una fecha válida.</p>}{!chronological && <p className="form-error" role="alert">Las fechas de las cuotas deben estar en orden cronológico.</p>}{!noSundays && <p className="form-error" role="alert">No se permiten cuotas en domingo.</p>}{differenceCents !== 0 && <p className="form-error" role="alert">La diferencia debe ser cero antes de guardar.</p>}{error && <p className="form-error" role="alert">{error}</p>}
