...
import { ChevronLeft, ChevronRight, Download, Printer, Search, X } from 'lucide-react'
import { listarClientes, obtenerAnalisisFinanciero } from '../application/clientes.use-cases'
import { generarAnalisisFinancieroClientePdf } from '../application/analisis-financiero-cliente-pdf'
import { clienteErrorMessage } from '../domain/cliente.error'
import type { AnalisisFinancieroResponse, Cliente, ClienteFilters, ClientePage } from '../domain/cliente.types'
...
export function AnalisisFinancieroClientePage() {
  ...
  const [loading, setLoading] = useState(false); const [error, setError] = useState(''); const [exportingPdf, setExportingPdf] = useState(false); const [statementStates, setStatementStates] = useState<Record<number, { loading: boolean; error: boolean }>>({}); const requestId = useRef(0)
  ...
  const exportPdf = async () => { if (!cliente || !analysis || exportingPdf) return; setExportingPdf(true); setError(''); try { await generarAnalisisFinancieroClientePdf(analysis, cliente) } catch { setError('No se pudo generar el PDF del análisis financiero.') } finally { setExportingPdf(false) } }
  return <section className="analisis-financiero-page"><div className="page-heading"><div><p className="eyebrow">CLIENTES</p><h1>Análisis financiero del cliente</h1><p className="muted">Consultá el comportamiento financiero completo de un cliente.</p></div></div>
    {!cliente && <div className="panel analisis-initial-panel"><div className="analisis-initial-heading"><h2>Selecciona un cliente para iniciar</h2></div><p className="analisis-initial-placeholder">Busca y selecciona un cliente para consultar su resumen financiero y sus préstamos.</p><div className="analisis-initial-action"><ClientePicker onSelect={selectClient} /></div></div>}
     {cliente && <div className="panel analisis-client-header"><div><h2>{nombreCliente(cliente)}</h2><p><strong>Identificación:</strong> {cliente.identificacion}</p></div><div className="analisis-contact"><span>Tel. principal: {cliente.telefono1}</span>{cliente.telefono2 && <span>Secundario: {cliente.telefono2}</span>}</div><button type="button" className="secondary-button" onClick={() => { setCliente(null); setAnalysis(null); setError(''); setStatementStates({}) }}>Cambiar cliente</button><button type="button" className="secondary-button" disabled={!analysis || exportingPdf} onClick={() => void exportPdf()}><Download size={15} /> {exportingPdf ? 'Generando...' : 'Exportar PDF'}</button></div>}
    ...
    {analysis && <><section className="analisis-summary"><h2>Resumen financiero</h2><div className="analisis-indicators" aria-label="Resumen financiero">{indicatorLabels.map(([key, label]) => <div className={`indicator-card${key === 'pendiente' ? ' indicator-card-pending' : ''}`} key={key}><span>{label}</span><strong>{key === 'cantidadPrestamos' ? analysis.resumen[key] : key === 'ganancia' ? formatCRC(analysis.resumen.ganancia) : formatCRC(analysis.resumen[key])}</strong></div>)}</div></section>{analysis.prestamos.length === 0 ? <div className="panel state-box analisis-empty">Este cliente no tiene préstamos registrados.</div> : <section className="analisis-loans-section"><h2 id="analisis-loans-title">Préstamos</h2><div className="table-wrap analisis-loans-table-wrap"><table className="analisis-loans-table" aria-labelledby="analisis-loans-title"><thead><tr><th scope="col">Préstamo</th><th scope="col">Estado</th><th scope="col">Cobranza</th><th scope="col">Capital</th><th scope="col">Pagado</th><th scope="col">Pendiente</th><th scope="col">Ganancia</th><th scope="col">Último pago</th><th scope="col">Duración</th><th scope="col">Acción</th></tr></thead><tbody>{analysis.prestamos.map((p) => { const statementState = statementStates[p.id] ?? { loading: false, error: false }; return <tr key={p.id}><td><strong>#{p.id}</strong></td><td><span className={estadoClass(p.estado)}>{p.estado}</span></td><td><span className={cobranzaClass(p.indicadorCobranza)}>{cobranzaLabel(p.indicadorCobranza)}</span></td><td>{formatCRC(p.capital)}</td><td>{formatCRC(p.totalPagado)}</td><td className="analisis-loan-pending"><strong>{formatCRC(p.saldoPendiente)}</strong></td><td>{formatCRC(p.interesPagado)}</td><td>{p.ultimoPago ? <span className="analisis-last-payment"><span>{dateLabel(p.ultimoPago.fecha)}</span><strong>{formatCRC(p.ultimoPago.monto)}</strong></span> : 'Sin pagos'}</td><td><span className="analisis-duration"><strong>{p.duracionDias !== null && p.duracionDias !== undefined ? `${p.duracionDias} días` : '—'}</strong></span></td><td><div className="analisis-statement-action"><button type="button" className="secondary-button" disabled={statementState.loading} onClick={() => void statement(p.id)}><Printer size={15} /> {statementState.loading ? 'Generando...' : statementState.error ? 'Reintentar' : 'Estado de cuenta'}</button>{statementState.error && <p className="form-error" role="alert">No se pudo generar el estado de cuenta.</p>}</div></td></tr> })}</tbody></table></div></section>}</>}
  </section>
}
