...
import type { ActualizarClienteInput, Cliente, ClienteFilters, ClientePage, ClienteRepository, CrearClienteInput } from '../domain/cliente.types'
...
export class AxiosClienteRepository implements ClienteRepository {
  ...
  private multipart(input: CrearClienteInput | ActualizarClienteInput): FormData { const form = new FormData(); Object.entries(input).forEach(([key, value]) => { if (key === 'identificacionFile') { if (value instanceof File) form.append('identificacionFile', value); return } if (value !== null && value !== undefined) form.append(key, String(value)); else form.append(key, '') }); return form }
