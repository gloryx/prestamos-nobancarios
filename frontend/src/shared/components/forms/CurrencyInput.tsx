...
function parseInput(input: string): { value: number | null; text: string } {
  ...
  const separatorIndex =
    lastComma >= 0 ? lastComma : dotIsGrouping ? -1 : lastDot;
  const trailingDigits =
    separatorIndex < 0 ? "" : cleaned.slice(separatorIndex + 1);
  const isDecimal =
    separatorIndex >= 0 && (lastComma >= 0 || trailingDigits.length <= 2);
  const integerPart =
    separatorIndex >= 0 && isDecimal
  ...
  if (!Number.isFinite(value) || decimalPart.length > 2)
    return { value: null, text: "" };

  const formattedInteger = numberFormatter.format(Number(digits));
  return {
    value,
    text: `${formattedInteger}${isDecimal ? `,${decimalPart}` : ""}`,
  };
}
        ...
        <input
          ...
          onChange={(event) => {
            ...
            requestAnimationFrame(() => {
              ...
              if (caret === event.target.value.length)
                input.setSelectionRange(parsed.text.length, parsed.text.length);
              else {
                let seen = 0;
                let position = 0;
                while (
