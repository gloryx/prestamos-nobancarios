...
export class CrearRefinanciamientoUseCase {
  ...
  async execute(dto: CrearRefinanciamientoDto, actorUsuarioId?: number) {
    ...
    return this.dataSource.transaction(async manager => {
      ...
      const nuevo = new Prestamo(null, origen.clienteId, dto.periodicidadPagoId, dto.formaPagoId, null, date(dto.fecha), capitalPrestamoNuevo, interesTotalPrestamoNuevo, montoTotalPrestamoNuevo, dineroNuevoDesembolsado, dto.cantidadPagos, dto.planPersonalizado, EstadoPrestamo.ACTIVO, dto.observaciones?.trim() || null, new Date(), new Date());
      const savedNuevo = await manager.getRepository(PrestamoOrmEntity).save(PrestamoMapper.toOrm(nuevo));
      const nuevoDomain = Object.assign(new Prestamo(savedNuevo.id, savedNuevo.clienteId, savedNuevo.periodicidadPagoId, savedNuevo.formaPagoId, null, date(savedNuevo.fechaAlta), savedNuevo.capital, savedNuevo.interes, savedNuevo.montoTotal, savedNuevo.montoDesembolsado, savedNuevo.cantidadPagos, savedNuevo.planPersonalizado, savedNuevo.estado, savedNuevo.observaciones, savedNuevo.fechaCreacion, savedNuevo.fechaActualizacion), { cliente: { id: cliente.id, nombre: '', identificacion: cliente.identificacion }, periodicidadPago: { id: periodicidad.id, nombre: periodicidad.nombre }, formaPago: { id: forma.id, nombre: forma.nombre }, formaDesembolso: null });
