...
export interface CadenaResponse { prestamoRaizId: number; prestamoTerminalId: number; fechaInicio: string; resumen: { cantidadPrestamos: number; cantidadRefinanciamientos: number; capitalInicial: number; capitalTerminal: number; totalCapitalTrasladado: number; totalDineroNuevoDesembolsado: number; totalInteresNuevoPactado: number; diasGanadosAcumulados: number; diasGanadosCompletos: boolean; fechaUltimoRefinanciamiento: string; }; prestamos: CadenaPrestamoResponse[]; transiciones: TransicionCadenaResponse[]; }
export interface CadenasClienteResponse { cliente: ReturnType<typeof clienteResponse>; convencionOrdenFechaInicio: string; resumen: { cantidadCadenas: number; cantidadRefinanciamientos: number; totalCapitalTrasladado: number; totalDineroNuevoDesembolsado: number; totalInteresNuevoPactado: number; diasGanadosAcumulados: number; diasGanadosCompletos: boolean; }; cadenas: CadenaResponse[]; }
