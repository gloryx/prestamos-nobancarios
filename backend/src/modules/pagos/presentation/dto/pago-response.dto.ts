...
export class PagoResponseDto {
  ...
  @ApiPropertyOptional({ example: 2, nullable: true }) numeroPago: number | null;
  @ApiPropertyOptional({ example: 2, nullable: true, description: 'Legacy alias of numeroPago.' }) numeroCuota: number | null;
  @ApiPropertyOptional({ example: '2026-09-30', format: 'date', nullable: true }) fechaVencimiento: string | null;
  ...
  @ApiPropertyOptional({ nullable: true }) usuarioAnulacion: { id: number; nombreCompleto: string } | null;
  ...
  @ApiProperty({ example: { id: 1, identificacion: '1-1111-1111', nombreCompleto: 'JUAN PEREZ', telefonoPrincipal: '8888-8888' } }) cliente: { id: number; identificacion?: string; nombreCompleto?: string; telefonoPrincipal?: string | null };
...
export class PagosTotalesResponseDto { @ApiProperty() cantidadPagos: number; @ApiProperty() totalRecibido: number; @ApiProperty() capitalAplicado: number; @ApiProperty() interesAplicado: number; }
export class PagosPaginadosResponseDto { @ApiProperty({ type: [PagoResponseDto] }) datos: PagoResponseDto[]; @ApiProperty() pagina: number; @ApiProperty() limite: number; @ApiProperty() total: number; @ApiProperty() totalPaginas: number; @ApiProperty({ type: PagosTotalesResponseDto, description: 'Siempre calculado exclusivamente con pagos REGISTRADO.' }) totales: PagosTotalesResponseDto; }
