import { Body, Controller, Get, Param, ParseIntPipe, Post, Query, Req, Optional } from '@nestjs/common';
...
const response = (pago: PagoConRelaciones): PagoResponseDto => ({ id: pago.id!, prestamoId: pago.prestamoId, formaPagoId: pago.formaPagoId, monto: pago.monto, capitalAplicado: pago.capitalAplicado, interesAplicado: pago.interesAplicado, cobradorId: pago.cobradorId, fecha: pago.fecha.toISOString().slice(0, 10), fechaCreacion: pago.fechaCreacion, observaciones: pago.observaciones, planPagoId: pago.planPagoId ?? null, numeroCuota: pago.numeroCuota ?? null, estado: pago.estado, formaPagoNombre: pago.formaPago?.nombre ?? '', puedeAnular: pago.puedeAnular === true, anulacion: pago.anulacion ?? null, fechaAnulacion: pago.fechaAnulacion, usuarioAnulacionId: pago.usuarioAnulacionId, motivoAnulacion: pago.motivoAnulacion, observacionAnulacion: pago.observacionAnulacion, formaPago: pago.formaPago, prestamo: pago.prestamo, cliente: pago.cliente, cobrador: pago.cobrador });
...
export class PagosController {
  ...
  @Get('cobros-del-dia') @ApiOperation({ summary: 'Consultar cobros del día o rango de fechas', description: 'Returns one row per payment-plan installment from the operational plan of active loans. Dates are date-only and inclusive.' }) @ApiQuery({ name: 'fecha', required: false, type: String, example: '2026-09-15' }) @ApiQuery({ name: 'fechaDesde', required: false, type: String, example: '2026-09-15' }) @ApiQuery({ name: 'fechaHasta', required: false, type: String, example: '2026-09-30' }) @ApiResponse({ status: 200, type: CobrosDelDiaResponseDto })
