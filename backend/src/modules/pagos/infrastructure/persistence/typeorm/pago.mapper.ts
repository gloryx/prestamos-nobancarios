...
export class PagoMapper {
  static toDomain(entity: PagoOrmEntity, puedeAnular = false): PagoConRelaciones {
    const pago = new Pago(entity.id, entity.prestamoId, entity.formaPagoId, entity.monto, entity.capitalAplicado, entity.interesAplicado, entity.cobradorId, dateFromDb(entity.fecha), entity.observaciones, entity.fechaCreacion, entity.planPagoId, entity.redistribuyoPlan, entity.estado);
     ...
     const planPago = entity.planPago ? { id: entity.planPago.id, numeroPago: entity.planPago.numeroPago, fechaVencimiento: entity.planPago.fechaVencimiento } : null;
     const auditRelation = audit ? { fecha: new Date(`${audit.fecha}T00:00:00.000Z`), motivo: audit.motivo, observacion: audit.observacion, usuario: audit.usuario ? { id: audit.usuario.id, nombreCompleto: audit.usuario.nombreCompleto } : undefined } : null;
      if (!entity.formaPago || !entity.prestamo) return Object.assign(pago, compatibility, { planPagoId: entity.planPagoId, numeroPago: planPago?.numeroPago ?? null, numeroCuota: planPago?.numeroPago ?? null, planPago, anulacion: auditRelation, puedeAnular }) as unknown as PagoConRelaciones;
      return Object.assign(pago, compatibility, { formaPago: { id: entity.formaPago.id, nombre: entity.formaPago.nombre }, prestamo: { id: entity.prestamo.id, estado: entity.prestamo.estado, capital: entity.prestamo.capital, interes: entity.prestamo.interes, montoTotal: entity.prestamo.montoTotal } as PrestamoPagoRelacion, cliente: entity.prestamo.cliente ? { id: entity.prestamo.cliente.id, identificacion: entity.prestamo.cliente.identificacion, nombreCompleto: [entity.prestamo.cliente.primerNombre, entity.prestamo.cliente.segundoNombre, entity.prestamo.cliente.primerApellido, entity.prestamo.cliente.segundoApellido].filter(Boolean).join(' '), telefonoPrincipal: entity.prestamo.cliente.telefono1 } : undefined, cobrador: entity.cobrador ? { id: entity.cobrador.id, identificacion: entity.cobrador.identificacion, nombreCompleto: entity.cobrador.nombreCompleto, telefono: entity.cobrador.telefono, correo: entity.cobrador.correo } : { id: entity.cobradorId, identificacion: '', nombreCompleto: '', telefono: null, correo: null }, planPagoId: entity.planPagoId, numeroPago: planPago?.numeroPago ?? null, numeroCuota: planPago?.numeroPago ?? null, planPago, anulacion: auditRelation, puedeAnular }) as PagoConRelaciones;
  }
