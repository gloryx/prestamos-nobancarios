...
export class PagoTypeOrmRepository implements PagoRepository {
  ...
  async listarPorPrestamo(prestamoId: number): Promise<PagoConRelaciones[]> { const entities = await this.withRelations().where('pago.prestamo_id = :prestamoId', { prestamoId }).andWhere('pago.estado = :estado', { estado: 'REGISTRADO' }).orderBy('pago.fecha', 'ASC').addOrderBy('pago.id', 'ASC').getMany(); return entities.map(PagoMapper.toDomain); }
