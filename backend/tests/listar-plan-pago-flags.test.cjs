...
const execute = async (estado, payments = []) => {
  ...
  const editability = {
    ...
    calcularFlags(numeroPago, protectedIds, id, loanState, lastProtectedNumber, lastOperationalNumber = numeroPago) { const protegida = protectedIds.has(id); const editable = loanState === EstadoPrestamo.ACTIVO && !protegida && numeroPago > lastProtectedNumber; const puedeEditarFecha = loanState === EstadoPrestamo.ACTIVO && (editable || numeroPago === lastOperationalNumber); return { protegida, editable, puedeEditarFecha, puedeEditarMonto: editable, eliminable: editable }; },
  };
...
test('last protected operational installment exposes date-only capability', async () => { const { result } = await execute(EstadoPrestamo.ACTIVO, [payment(1), payment(3)]); assert.deepEqual([result[1].editable, result[1].puedeEditarFecha, result[1].puedeEditarMonto], [false, false, false]); assert.deepEqual([result[2].protegida, result[2].editable, result[2].puedeEditarFecha, result[2].puedeEditarMonto, result[2].eliminable], [true, false, true, false, false]); });
